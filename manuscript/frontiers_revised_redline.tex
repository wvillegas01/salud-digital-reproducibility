...
\section{}
Machine learning-based clinical prediction models are typically evaluated under intra-domain conditions, if training and test data share the same distribution. However, in real-world settings, these models operate in heterogeneous contexts where differences in populations, clinical protocols, and acquisition processes introduce dataset shifts, leading to performance degradations that are not captured by aggregate metrics such as accuracy or AUC-ROC. This work addresses this limitation through a multi-environment evaluation approach that integrates the MIMIC and eICU datasets into a common feature space and evaluates models such as Logistic Regression, Random Forest, and Gradient Boosting under intra-domain and cross-transfer conditions. \textcolor{red}{After peer review, the analysis was reorganized around a clinically prespecified 24-hour landmark window and a strict shared-feature predictor set as the most defensible primary specification. The original full-episode/full-feature analysis is retained only as a retrospective secondary analysis because it can include measurements recorded after a clinically plausible prediction time and because it used eICU-specific predictors that are not available in MIMIC.} A \textcolor{red}{decision-level diagnostic analysis} links the predicted probability distribution to the application of a fixed threshold, allowing for interpretation of model behavior beyond global metrics. \textcolor{red}{In the primary 24-hour shared-feature analysis, cross-domain discrimination was weak to modest and threshold-dependent performance was unstable. For Train eICU $\rightarrow$ Test MIMIC, Random Forest generated no positive predictions at $\tau=0.5$ (AUC-ROC 0.583, F1-score 0.000), Logistic Regression achieved very high sensitivity only by predicting almost all MIMIC cases as positive (recall 0.971, specificity 0.011), and Gradient Boosting recovered limited sensitivity (recall 0.235, F1-score 0.320).} \textcolor{red}{The original full-episode analysis produced much higher internal eICU performance for tree-based models, including Random Forest AUC-ROC 0.993, but those values are interpreted as retrospective and potentially inflated by full-stay aggregation and eICU-specific variables rather than as prospective deployment estimates.} These results show that \textcolor{red}{the observed cross-domain differences were associated with substantial fixed-threshold decision instability, even when partial ranking discrimination was retained}.
...
Once the variables are selected, the features are organized in a common space that includes physiological descriptors observed in both MIMIC and eICU. \textcolor{red}{The primary 24-hour shared-feature model used age, gender, heart-rate mean/minimum/maximum, temperature mean/minimum/maximum, respiratory-rate mean/minimum/maximum, systolic blood-pressure mean/minimum/maximum, diastolic blood-pressure mean/minimum/maximum, glucose mean/minimum/maximum, creatinine mean/minimum/maximum, white-blood-cell mean/minimum/maximum, sodium mean/minimum/maximum, and blood-urea-nitrogen mean/minimum/maximum. Source-specific predictors were excluded from the primary transfer models, including \texttt{hospitalid}, \texttt{apachescore}, \texttt{predictedhospitalmortality}, and any variable completely absent in one domain. This is essential because \texttt{predictedhospitalmortality} is itself an APACHE-derived mortality estimate in eICU and would constitute a predictor derived for the same endpoint. The full-episode/full-feature model that retained these eICU-specific variables is therefore reported only as a retrospective secondary analysis and not as the primary evidence for cross-domain transportability.}
...
\textcolor{red}{The primary retained variable set no longer assumes that a 29-column structural matrix is equivalent to clinical harmonization. Instead, the primary model uses only variables observed in both sources during the first 24 hours. The original 29-variable full-episode matrix is described as a secondary retrospective specification. In that matrix, completeness was not identical across sources: \texttt{hospitalid}, \texttt{apachescore}, \texttt{predictedhospitalmortality}, and some pressure summaries were not equivalent across MIMIC and eICU. Conversely, core vital-sign and laboratory summaries were present in both sources with low-to-moderate missingness. This pattern indicates that structural column alignment alone is insufficient as evidence of measurement equivalence.}
...
\begin{figure}[h!]
	...
	\end{center}
\caption{Distribution of key clinical variables across datasets. (a) Heart rate distribution for MIMIC and eICU in beats per minute. (b) Glucose level distribution for MIMIC and eICU in mg/dL. (c) Creatinine distribution for MIMIC and eICU in mg/dL. (d) Mean arterial pressure distribution in mmHg for the subset with available values. \textcolor{red}{Kernel-density curves were generated from available non-missing values using the plotting library's default Gaussian kernel bandwidth. Mean arterial pressure was not retained in the primary 24-hour shared-feature model because its availability and extraction were not equivalent across sources.}}\label{fig:fig3}
\end{figure}
...
Figure~\ref{fig:fig3}(d), corresponding to mean arterial pressure, \textcolor{red}{is retained only as a descriptive completeness-limited plot and not as evidence of harmonized cross-domain availability.} The curve exhibits unimodal behavior, with moderate dispersion centered on intermediate values. The density increases progressively, reaching a maximum around 75--80, then gradually decreases toward higher values, with no evidence of pronounced extreme tails.
...
\textcolor{red}{As a secondary diagnostic for the original full-episode/full-feature specification, we also evaluated calibration and post hoc threshold sensitivity. This analysis is not treated as an operational threshold-selection procedure because the F1-maximizing thresholds were selected using the labeled test sets; it is therefore an oracle/post hoc diagnostic used only to assess whether ranking information remained below the transported 0.5 threshold. In Train eICU $\rightarrow$ Test MIMIC, average precision was 0.541 for Random Forest and 0.559 for Gradient Boosting, confirming that some ranking information remained despite zero positive predictions at $\tau=0.5$. Calibration gaps were nevertheless large: five-bin expected calibration error was 0.258 for Random Forest, 0.285 for Gradient Boosting, and 0.183 for Logistic Regression. Calibration intercept/slope were 1.964/1.030 for Random Forest, 1.886/0.802 for Gradient Boosting, and -0.542/0.082 for Logistic Regression. Post hoc F1-maximizing thresholds were much lower than 0.5 for the tree-based models (0.067 for Random Forest and 0.027 for Gradient Boosting), recovering recall values of 0.783 and 0.826, respectively, but with only moderate precision (0.480 and 0.469) and specificity near 0.56 and 0.52. These results indicate that the transferred models were not devoid of all ordering information; rather, their probability scale was poorly aligned with the transported fixed threshold.}

\textcolor{red}{The full-feature 24-hour landmark sensitivity analysis, which retained variables when structurally present in the broader feature table, is shown in Table~\ref{tab:landmark_24h_transfer}. Its results were similar to the primary shared-feature landmark analysis for Logistic Regression and Random Forest, but Gradient Boosting had lower positive-class activation in eICU-to-MIMIC transfer (2 true positives, recall 0.059, F1-score 0.100, AUC-ROC 0.535). This reinforces the conclusion that Gradient Boosting behavior is sensitive to the exact feature-alignment schema, whereas Random Forest consistently shows very limited fixed-threshold positive-class activation in the eICU-to-MIMIC direction.}
...
\textcolor{red}{The main contribution of this work lies in integrating a multi-environment evaluation protocol with decision-level diagnostic analysis. Unlike approaches focused only on global metrics, this study identifies conditions under which a transported threshold loses operational meaning, thereby linking the shape of the predicted-probability distribution to the structure of the confusion matrix.} This approach is particularly relevant in clinical applications, where error interpretation and the ability to detect positive events are more critical than average performance.
...
\textcolor{red}{This work uses a multi-environment evaluation scheme that distinguishes between intra-domain performance and transfer behavior, and a decision-level diagnostic analysis that explicitly links probabilistic estimation to the final decision. This integration enables identification of specific conditions under which a model retains partial ranking ability but fails to activate the positive class under a transported threshold.}
