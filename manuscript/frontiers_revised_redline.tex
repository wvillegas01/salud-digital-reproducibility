...
%DIF DEL C:\Users\wilop\Documents\Codex\2026-09-06\ha\work\today_redline\old_clean.tex   Wed Sep 16 08:35:40 2026
%DIF ADD C:\Users\wilop\Documents\Codex\2026-09-06\ha\work\today_redline\new_clean.tex   Wed Sep 16 08:35:40 2026
...
Empirical evidence  \DIFadd{indicates }that clinical models \DIFadd{can }experience substantial degradation when evaluated in populations not seen during training.~\citet{Musa2026} report that the performance of deep learning models can decrease by 10~\%--25~\% when applied across different populations,  \DIFadd{consistent with the }impact of domain shift on generalizability. This behavior is attributed to differences in disease prevalence, demographic characteristics, and data collection processes. Furthermore, mitigation strategies, such as domain adaptation and transfer learning, have been observed to offer limited improvements, with performance increases ranging from 5~\% to 15~\%, indicating that the problem is not fully resolved by conventional techniques.
...
\begin{table}[h]
	...
	\begin{tabular}{lllll}
		\hline
		\textbf{Feature family} & \textbf{MIMIC source/items} & \textbf{Specimen or field restriction} & \textbf{eICU source/name} & \textbf{Unit handling} \\ \hline
		Glucose & LABEVENTS 50809, 50931 & blood/plasma/serum laboratory glucose & lab: \texttt{glucose} & mg/dL retained \\
		Creatinine & LABEVENTS 50912 & blood/serum/plasma creatinine only; urinary and protein/creatinine ratio excluded & lab: \texttt{creatinine} & mg/dL retained \\
		White blood cell count & LABEVENTS 51301 & blood WBC count & lab: \texttt{wbc x 1000} & source-aligned count scale retained \\
		Hematocrit & LABEVENTS 50810, 51221 & blood hematocrit & lab: \texttt{hct} & percent retained \\
		Sodium & LABEVENTS 50824, 50983 & blood/plasma/serum sodium & lab: \texttt{sodium} & mEq/L retained \\
		Blood urea nitrogen & LABEVENTS 51006 & blood urea nitrogen & lab: \texttt{bun} & mg/dL retained \\
		Heart rate & CHARTEVENTS 211, 220045 & charted vital sign & structured vital-period fields & beats/min retained \\
		Respiratory rate & CHARTEVENTS 618, 220210 & charted vital sign & structured vital-period fields & breaths/min retained \\
		Blood pressure & CHARTEVENTS 51, 442, 455, 52, 456, 8368, 8440, 8441, 220050--220052, 220179--220181, 224167, 224643, 225309--225312, 227242, 227243 & systolic, diastolic, and mean arterial pressure fields & structured vital-period fields & mmHg retained \\
		Temperature & CHARTEVENTS 676, 677, 223762, 226329; Fahrenheit 678, 679, 223761 & charted temperature; audited but excluded from primary predictors & structured temperature fields when available & Fahrenheit converted to Celsius for audit \\ \hline
	\end{tabular}%
	}
\end{table}
...
The application of the threshold $\tau$ induces a partitioning of the decision space whose stability depends  on the structure \DIFadd{and calibration }of the probability distribution \DIFadd{, event prevalence, and the location of the operating point. Well-separated distributions may still lead to conservative or liberal decisions if the fixed threshold is poorly aligned with the score scale. Conversely, overlapping or shifted distributions can generate }dominance of one \DIFadd{predicted }class, thereby affecting  \DIFadd{threshold-dependent detection of positive events even when some rank-ordering information remains}.

The structural representation of this process is shown in Figure  \DIFadd{2}, which formalizes the sequence  \DIFadd{from probabilistic modeling to fixed-threshold classification}. The scheme  \DIFadd{represents in-domain and cross-domain evaluation as empirical checks of probability distributions and operating-point stability rather than as guaranteed decision outcomes. Possible observed behaviors include stable activation, conservative decisions, liberal decisions, or degenerate decisions}.

\begin{figure}[h!]
	\begin{center}
		\includegraphics[width=15cm]{Fig2.jpg}% This is a *.eps file
	\end{center}
	\caption{Decision-level  \DIFaddFL{diagnostic framework for evaluating probability distributions and fixed operating-point stability}.}\label{fig:fig2}
\end{figure}
...
\subsection{Characterization of the integrated dataset and selection of variables}
 \DIFadd{Table 1 presents the primary 24-hour landmark cohort used for the main analysis. The primary analyzable cohort contained 110 MIMIC ICU stays from 79 patients and 105 admissions, with 33 in-hospital deaths (30.0\%)},  \DIFadd{and 1}, \DIFadd{324 eICU ICU stays from 1,124 patients and 1,277 hospital-system stays, }with  \DIFadd{117 in-hospital deaths (8.8\%). The secondary full-episode supervised cohort was larger (136 MIMIC ICU stays and }3, \DIFadd{676 eICU ICU stays), but it is not used as the main cohort table because the primary analysis is defined by 24-hour landmark eligibility and strict shared-feature retention}.

 \begin{table}[h]
	\color{red}
	\caption{\DIFaddFL{Primary 24-hour landmark cohort composition across MIMIC and eICU}}
	\label{table:integrated_dataset_structure}
	\setlength{\tabcolsep}{2pt}
	\tiny
	\resizebox{\textwidth}{!}{%
	\begin{tabular}{p{140pt}llllllll}
		\hline
		\textbf{Cohort stage} & \textbf{MIMIC patients} & \textbf{MIMIC admissions} & \textbf{MIMIC ICU stays} & \textbf{MIMIC deaths} & \textbf{eICU patients} & \textbf{eICU hospital stays} & \textbf{eICU ICU stays} & \textbf{eICU deaths} \\ \hline
		Full-episode supervised cohort with outcome & NR & NR & 136 & 46 & NR & NR & 3,676 & 310 \\
		Eligible for 24-hour landmark analysis & 79 & 105 & 110 & 33 & 1,124 & 1,277 & 1,324 & 117 \\
		Primary shared-feature analyzable cohort & 79 & 105 & 110 & 33 & 1,124 & 1,277 & 1,324 & 117 \\ \hline
	\end{tabular}%
	}
	\footnotesize
\end{table}

\DIFadd{NR indicates that patient-level and hospital-system-stay counts were not retained in the aggregate secondary full-episode summary. The primary landmark cohort is therefore the only cohort for which all grouping levels are reported. The MIMIC landmark cohort includes repeated ICU stays for some patients (110 ICU stays from 79 patients), and the eICU landmark cohort also includes repeated ICU stays (1}, \DIFadd{324 ICU stays from 1,124 patients)}.

Table  \DIFadd{2 summarizes the quantitative effect of replacing }the secondary full-episode \DIFadd{/full-feature workflow with the corrected primary 24-hour shared-feature workflow. The table is not intended to compare two deployable models; it documents how sample size, event counts, discrimination, calibration, and threshold-dependent behavior changed after the landmark restriction, ICU-stay-level MIMIC aggregation, removal of endpoint-derived/source-specific predictors, and strict shared-feature retention}.

\begin{table}[h]
	\color{red}
	\caption{ \DIFaddFL{Comparison between secondary full-episode workflow }and  \DIFaddFL{corrected primary 24-hour shared-feature workflow}}
	...
	\begin{tabular}{llll}
		...
		eICU ICU stays & 3,676 & 1,324 & Landmark eligibility reduced eICU denominator \\
		...
		GB eICU$\rightarrow$MIMIC AUC & 0.692 & 0.609 & lower corrected AUC \\
...
Regarding variable completeness, the  \DIFadd{primary }retained variable set does not assume that a 29-column structural matrix is equivalent to clinical harmonization. Instead, the primary 24-hour model uses 35 pre-encoding predictors derived from 11 shared clinical variable families plus age and gender. Temperature was not retained in the primary shared-feature matrix because eICU missingness was 91.8\% after applying the \DIFadd{corrected }24-hour landmark definition. The secondary 29-variable full-episode matrix is described as a retrospective specification.  \DIFadd{Core }vital-sign and laboratory summaries were present in both sources with low-to-moderate missingness\DIFadd{, whereas source-specific fields and insufficiently observed variables were excluded}. This pattern indicates that structural column alignment alone is insufficient as evidence of measurement equivalence.
...
\begin{table}[h]
	...
	\begin{tabular}{lllllll}
		...
		\color{red} eICU internal validation & LogisticRegression & 0.743 & 0.189 & 0.586 & 0.286 & 0.715 \\
		eICU internal validation & RandomForest & 0.912 & NA & 0.000 & 0.000 & 0.765 \\
		eICU internal validation & GradientBoosting & 0.897 & 0.222 & 0.069 & 0.105 & 0.734 \\
		...
		Integrated 5-fold CV & GradientBoosting & $0.884 \pm 0.020$ & $0.377 \pm 0.290$ & $0.080 \pm 0.050$ & $0.130 \pm 0.086$ & $0.708 \pm 0.062$ \\ \hline
...
In the primary 24-hour shared-feature internal validation scenario on eICU, apparent performance was much more modest than in the secondary full-episode analysis. Logistic Regression produced the highest recall (0.586) but low precision (0.189), indicating many false-positive activations. Random Forest achieved high accuracy (0.912) and moderate AUC-ROC (0.765), but generated no positive predictions at the fixed 0.5 threshold; precision is therefore reported as NA rather than zero because TP+FP=0. Gradient Boosting showed similar discrimination (AUC-ROC 0.734) but very low recall (0.069).

In integrated five-fold cross-validation, AUC-ROC values remained in the range 0.708--0.717, but threshold-dependent minority-class metrics were weak for tree-based models. Random Forest had recall $0.007 \pm 0.013$ and F1-score $0.013 \pm 0.026$, while Gradient Boosting had recall $0.080 \pm 0.050$ and F1-score $0.130 \pm 0.086$. Logistic Regression again favored sensitivity over precision, with recall $0.600 \pm 0.070$ and precision $0.196 \pm 0.023$. These values indicate that the clinically stricter landmark/shared-feature design substantially reduces the apparent performance observed in the secondary full-episode/full-feature results.
...
\begin{table}[h]
	...
	\begin{tabular}{p{140pt}p{50pt}p{50pt}p{50pt}p{45pt}p{45pt}p{60pt}}
		...
		\color{red} Train eICU $\rightarrow$ Test MIMIC & Logistic & 0.564 & 0.353 & 0.545 & 0.429 & 0.598 \\
		Train eICU $\rightarrow$ Test MIMIC & RF & 0.700 & NA & 0.000 & 0.000 & 0.642 \\
		Train eICU $\rightarrow$ Test MIMIC & GB & 0.709 & 0.667 & 0.061 & 0.111 & 0.609 \\
		...
		Train MIMIC $\rightarrow$ Test eICU & GB & 0.755 & 0.112 & 0.256 & 0.156 & 0.555 \\ \hline

	\end{tabular}
	\footnotesize
\end{table}

In the reverse scenario, from Train MIMIC $\rightarrow$ Test eICU, no model achieved clinically strong positive-class detection. Logistic Regression detected 44 of 117 deaths but produced 422 false positives. Random Forest preserved high specificity (0.971) but detected only 4 of 117 deaths. Gradient Boosting detected 30 of 117 deaths but still had low precision (0.112) and F1-score (0.156). The comparison between directions supports a cautious interpretation: the central issue is weak transportable discrimination combined with instability of the probability scale and fixed-threshold decision rule.

Uncertainty in the primary transfer results was quantified using stratified non-parametric bootstrap confidence intervals based on 2,000 \DIFadd{episode-level }resamples within each test set. Bootstrap resamples preserved the observed event/non-event counts; no resamples were discarded for containing a single class. Sensitivity and specificity intervals were calculated using exact binomial confidence intervals because these quantities are conditional on the number of observed events or non-events and because a test-set bootstrap can otherwise produce misleading zero-width intervals when a model makes no positive predictions. \DIFadd{These intervals treat ICU episodes as the resampling unit and therefore do not fully account for repeated ICU stays within the same patient; this is a limitation because the MIMIC landmark cohort includes 110 ICU stays from 79 patients and the eICU landmark cohort includes 1,324 ICU stays from 1,124 patients. }Precision is reported as NA when TP+FP=0. In the Train eICU $\rightarrow$ Test MIMIC scenario, Random Forest had sensitivity 0.000 (95\% exact CI 0.000--0.106), specificity 1.000 (95\% exact CI 0.953--1.000), F1-score 0.000 (bootstrap CI 0.000--0.000), and AUC-ROC 0.642 (bootstrap CI 0.524--0.756). Gradient Boosting had sensitivity 0.061 (95\% exact CI 0.007--0.202), specificity 0.987 (95\% exact CI 0.930--1.000), F1-score 0.111 (bootstrap CI 0.000--0.263), and AUC-ROC 0.609 (bootstrap CI 0.482--0.735). Logistic Regression had sensitivity 0.545 (95\% exact CI 0.364--0.719), specificity 0.571 (95\% exact CI 0.454--0.684), F1-score 0.429 (bootstrap CI 0.313--0.549), and AUC-ROC 0.598 (bootstrap CI 0.479--0.719). These uncertainty results are summarized in Table  \DIFadd{5.
}

\begin{table}[h]
	\caption{Uncertainty summary for primary 24-hour shared-feature cross-domain transfer results. E2M denotes Train eICU $\rightarrow$ Test MIMIC; M2E denotes Train MIMIC $\rightarrow$ Test eICU. Sensitivity and specificity intervals are exact binomial 95\% confidence intervals; accuracy, F1-score, AUC-ROC, and Brier intervals are stratified \DIFaddFL{episode-level }bootstrap 95\% intervals from 2,000 resamples. Precision is reported as NA when TP+FP=0.}
...
\begin{table}[h]
	...
	\begin{tabular}{llllllllll}
		...
		eICU internal & GB & 0.060 & 0.983 & 0.097 & 0.743 & 0.211 & 0.081 & 0.037 & -0.703/0.587 \\
		...
		E2M transfer & GB & 0.061 & 0.987 & 0.111 & 0.609 & 0.467 & 0.241 & 0.219 & 0.109/0.354 \\
		...
		MIMIC internal & GB & 0.212 & 0.844 & 0.269 & 0.593 & 0.382 & 0.258 & 0.214 & -0.553/0.156 \\
		...
		M2E transfer & GB & 0.256 & 0.804 & 0.156 & 0.555 & 0.107 & 0.181 & 0.198 & -2.204/0.085 \\ \hline
	\end{tabular}%	
	}
\end{table}

Because the MIMIC source cohort is small (110 landmark ICU episodes with 33 deaths), internal MIMIC validation was treated as an unstable diagnostic rather than as definitive evidence of source-domain performance. In 20 repeats of stratified five-fold cross-validation, AUC-ROC means were 0.474 for Logistic Regression, 0.585 for Random Forest, and 0.552 for Gradient Boosting, with fold-level standard deviations of 0.137, 0.109, and 0.119, respectively. Random Forest produced zero positive predictions in 22 of 100 folds and Gradient Boosting in 1 of 100 folds, whereas Logistic Regression did not produce zero-positive folds. Patient-grouped five-fold internal validation further reduced apparent MIMIC performance  \DIFadd{and showed weak threshold-dependent behavior across models (Table 7)}. This analysis indicates that the MIMIC-to-eICU transfer results should be interpreted against a weak and unstable MIMIC source model, not as a clean demonstration of degradation from a strong source-domain baseline.

\begin{table}[h]
	\color{red}
	\caption{\DIFaddFL{Patient-grouped five-fold internal validation for the primary 24-hour shared-feature cohort}}
	...
	\begin{tabular}{llllllllll}
		...
		eICU & LR & 0.581 & 0.716 & 0.258 & 0.694 & 0.248 & 0.200 & 0.298 & 68/343/49/864 \\
		eICU & RF & 0.017 & 0.997 & 0.033 & 0.686 & 0.164 & 0.079 & 0.023 & 2/4/115/1203 \\
		eICU & GB & 0.094 & 0.982 & 0.147 & 0.703 & 0.196 & 0.082 & 0.044 & 11/22/106/1185 \\
		MIMIC & LR & 0.273 & 0.506 & 0.225 & 0.404 & 0.313 & 0.349 & 0.326 & 9/38/24/39 \\
		MIMIC & RF & 0.061 & 0.922 & 0.098 & 0.525 & 0.335 & 0.221 & 0.068 & 2/6/31/71 \\
		MIMIC & GB & 0.212 & 0.753 & 0.237 & 0.506 & 0.294 & 0.318 & 0.270 & 7/19/26/58 \\ \hline
	\end{tabular}%
	}
\end{table}

As a secondary diagnostic for the secondary full-episode/full-feature specification, calibration and post hoc threshold sensitivity were also evaluated \DIFadd{using the earlier five-quantile-bin diagnostic retained only for comparison with the secondary workflow}. This analysis is not treated as an operational threshold-selection procedure because the F1-maximizing thresholds were selected using the labeled test sets; it is therefore an oracle/post hoc diagnostic used only to assess whether ranking information remained below the fixed conventional 0.5 threshold. In Train eICU $\rightarrow$ Test MIMIC, average precision was 0.541 for Random Forest and 0.559 for Gradient Boosting, confirming that some ranking information remained despite zero positive predictions at $\tau=0.5$. Calibration gaps were nevertheless large: \DIFadd{the secondary }five-bin  \DIFadd{ECE }was 0.258 for Random Forest, 0.285 for Gradient Boosting, and 0.183 for Logistic Regression. Calibration intercept/slope were 1.964/1.030 for Random Forest, 1.886/0.802 for Gradient Boosting, and -0.542/0.082 for Logistic Regression. Post hoc F1-maximizing thresholds were much lower than 0.5 for the tree-based models (0.067 for Random Forest and 0.027 for Gradient Boosting), recovering recall values of 0.783 and 0.826, respectively, but with only moderate precision (0.480 and 0.469) and specificity near 0.56 and 0.52. These results indicate that the transferred models were not devoid of all ordering information; rather, their probability scale was poorly aligned with the fixed conventional threshold.

The full-feature 24-hour landmark sensitivity analysis, which retained variables when structurally present in the broader feature table, is shown in Table  \DIFadd{8. }Its results were similar to the primary shared-feature landmark analysis for Random Forest and Gradient Boosting, but Logistic Regression became more sensitive in eICU-to-MIMIC transfer (24 true positives, recall 0.727) at the cost of lower specificity (36 true negatives and 41 false positives). Gradient Boosting again  \DIFadd{produced three positive predictions, comprising two }true positives and  \DIFadd{one false positive. This supports a cautious interpretation }that fixed-threshold behavior is sensitive to feature-alignment decisions, whereas Random Forest repeatedly shows a highly conservative fixed-threshold operating point that is visible before and after transfer.

\begin{table}[h]
	\caption{Secondary full-feature 24-hour landmark sensitivity analysis for cross-domain transfer. E2M denotes Train eICU $\rightarrow$ Test MIMIC; M2E denotes Train MIMIC $\rightarrow$ Test eICU.}
...
Figure  \DIFadd{4 }presents the predicted probability distributions for the primary 24-hour shared-feature transfer analysis. In Figure  \DIFadd{4}(a), corresponding to Logistic Regression in the Train eICU $\rightarrow$ Test MIMIC scenario, the survival and death probability distributions overlap substantially around the fixed threshold. This explains why Logistic Regression retained partial sensitivity but also produced substantial false-positive burden in Table  \DIFadd{4.
}
...
In Figure  \DIFadd{4}(b), Random Forest probabilities remain below $\tau=0.5$ for all MIMIC test cases, which produces a constant negative classification despite an AUC-ROC above 0.5. In Figure  \DIFadd{4}(c), Gradient Boosting shows a less extreme pattern: a small subset of MIMIC cases crosses the threshold, producing limited recovery of positive-class activation.
...
\begin{table}[h]
	...
	\begin{tabular}{p{145pt}p{50pt}p{35pt}p{35pt}p{35pt}p{35pt}p{55pt}p{55pt}}
		...
		\color{red} Train eICU $\rightarrow$ Test MIMIC & Logistic & 44 & 33 & 15 & 18 & 0.545 & 0.571 \\
		...
		Train MIMIC $\rightarrow$ Test eICU & GB & 970 & 237 & 87 & 30 & 0.256 & 0.804 \\ \hline
...
\begin{figure}[h!]
	...
	\end{center}
	\caption{\DIFaddFL{Calibration curves for the primary 24-hour shared-feature analysis. (a) eICU internal out-of-fold validation. (b) Train eICU $\rightarrow$ Test MIMIC. (c) MIMIC internal out-of-fold validation. (d) Train MIMIC $\rightarrow$ Test eICU. Points represent non-empty equal-width probability bins; the dashed line indicates perfect calibration.}}\label{fig:fig5}
\end{figure}
...
Finally, this study does not perform hospital-level leave-site-out validation within eICU, and the MIMIC demo cohort is too small to support definitive population-level claims. Patient-grouped internal validation was conducted as a sensitivity analysis, but the cross-domain transfer  \DIFadd{uncertainty intervals remain }episode-level . The findings should therefore be interpreted as  \DIFadd{exploratory evidence consistent with threshold-specific instability under dataset shift }rather than as a definitive estimate of deployment performance  \DIFadd{or as identification of a unique causal mechanism}. Future work should use the full MIMIC and eICU resources, prespecified landmark times, patient-level de-duplication, \DIFadd{cluster-aware uncertainty estimation, }site-aware validation, and reporting aligned with clinical prediction-model guidance such as TRIPOD-AI and PROBAST-AI.

\section{Conclusions}
 \DIFadd{In these selected 24-hour landmark cohorts, the evaluated }clinical prediction models  \DIFadd{could not be interpreted adequately from aggregate discrimination metrics alone}. The primary evidence comes from a 24-hour landmark analysis restricted to variables observed with sufficient completeness in both MIMIC and eICU. Under this  stricter design, cross-domain discrimination was weak and threshold-dependent mortality detection was unstable across all models. Random Forest showed the strongest positive-class suppression in eICU-to-MIMIC transfer, but this fixed-threshold failure was already present during internal eICU validation. Gradient Boosting detected only two MIMIC deaths in eICU-to-MIMIC transfer, and Logistic Regression retained only partial sensitivity with substantial false-positive burden.

Logistic Regression maintained nonzero sensitivity in both transfer directions, but this did not constitute robust transportability. In the eICU-to-MIMIC direction, it detected 18 of 33 deaths but misclassified 33 of 77 non-events as deaths. In the reverse direction, it detected 44 of 117 deaths but generated 422 false positives.  \DIFadd{The study did not define clinical utility thresholds or evaluate net benefit, so these findings should be interpreted as unfavorable fixed-threshold operating behavior rather than as a formal clinical-utility assessment}.

The  \DIFadd{observed probability distributions were associated with the behavior of the fixed }decision threshold $\tau$ . When the probability mass  \DIFadd{was }concentrated in regions below the threshold, the classification process  \DIFadd{converged toward constant negative }decisions, regardless of the actual class. This  \DIFadd{pattern }explains the simultaneous presence of high specificity and the absence of positive-class detection in certain scenarios, which cannot be captured by metrics such as accuracy or AUC-ROC in isolation.
...
The results  \DIFadd{are consistent with the view }that integrating heterogeneous clinical datasets  introduces challenges related to variable alignment and distributional consistency. The difference in size, structure, and distribution between MIMIC and eICU  \DIFadd{was associated with misalignment patterns affecting model }behavior, even when common variables  \DIFadd{were }used. For this reason, future external-validation studies should privilege clinically harmonized variable dictionaries over purely structural column alignment.

 \DIFadd{Future work should evaluate }probabilistic calibration strategies  \DIFadd{and thresholds selected only from source-domain validation data or from prespecified clinical-utility criteria before external testing}. It is also relevant to explore  learning methods that explicitly integrate information from multiple domains during training  \DIFadd{while preserving a locked external-test protocol}. Extending the analysis to  \DIFadd{larger complete datasets, cluster-aware validation designs, }and prospective scenarios will allow  \DIFadd{a more reliable evaluation of generalizability}.
