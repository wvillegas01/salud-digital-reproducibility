...
%DIF DEL C:\Users\wilop\Dropbox\MPDI\2026\Salud-digital\Revisiones\salud-digital-reproducibility-package\manuscript\frontiers_revised.tex   Wed Sep 16 09:17:23 2026
%DIF ADD frontiers_clean.tex                                                                                                                Wed Sep 16 09:38:04 2026
...
Table 2 presents the \DIFaddbegin \DIFadd{cohort flow from the outcome-resolved full-episode cohort to the }\DIFaddend primary 24-hour landmark cohort used for the main analysis. The primary analyzable cohort contained 110 MIMIC ICU stays from 79 patients and 105 admissions, with 33 in-hospital deaths (30.0\%), and 1,324 eICU ICU stays from 1,124 patients and 1,277 hospital-system stays, with 117 in-hospital deaths (8.8\%). The \DIFaddbegin \DIFadd{flow explicitly shows that 26 }\DIFaddend MIMIC ICU stays and \DIFaddbegin \DIFadd{2,352 }\DIFaddend eICU ICU stays \DIFaddbegin \DIFadd{were excluded before landmark analysis because they did not remain observable through the first 24 hours or did not satisfy the corrected landmark-eligibility definition}\DIFaddend .

\begin{table}[h]
	\caption{\DIFdelFL{Primary }\DIFaddbeginFL \DIFaddFL{Cohort flow for the primary }\DIFaddendFL 24-hour landmark \DIFdelFL{cohort composition across MIMIC and eICU}\DIFaddbeginFL \DIFaddFL{analysis}\DIFaddendFL }
	\label{table:integrated_dataset_structure}
\DIFaddbeginFL \setlength{\tabcolsep}{4pt}
	\DIFaddendFL \footnotesize
\DIFaddbegin \begin{tabular}{p{165pt}p{58pt}p{58pt}p{58pt}p{58pt}}
		\hline
		\DIFadd{\textbf{Cohort stage} }& \DIFadd{\textbf{MIMIC ICU}}\newline \DIFadd{\textbf{stays} }& \DIFadd{\textbf{MIMIC}}\newline \DIFadd{\textbf{deaths} }& \DIFadd{\textbf{eICU ICU}}\newline \DIFadd{\textbf{stays} }& \DIFadd{\textbf{eICU}}\newline \DIFadd{\textbf{deaths} }\\ \hline
		\DIFadd{Outcome-resolved }\DIFaddend full-episode cohort \DIFaddbegin & \DIFadd{136 }& \DIFadd{46 }& \DIFadd{3,676 }& \DIFadd{310 }\\
		\DIFadd{Excluded before 24-hour landmark }& \DIFadd{26 }& \DIFadd{13 }& \DIFadd{2,352 }& \DIFadd{193 }\\
		\DIFadd{Primary 24-hour landmark cohort }& \DIFadd{110 }& \DIFadd{33 }& \DIFadd{1,324 }& \DIFadd{117 }\\
		\DIFadd{Unique patients in landmark cohort }& \DIFadd{79 }& \DIFadd{-- }& \DIFadd{1,124 }& \DIFadd{-- }\\
		\DIFadd{Admissions/hospital-system stays in landmark cohort }& \DIFadd{105 }& \DIFadd{-- }& \DIFadd{1,277 }& \DIFadd{-- }\\ \hline
	\end{tabular}
\end{table}

\DIFaddend The MIMIC landmark cohort includes repeated ICU stays for some patients (110 ICU stays from 79 patients), and the eICU landmark cohort also includes repeated ICU stays (1,324 ICU stays from 1,124 patients). \DIFaddbegin \DIFadd{These patient-group counts were used for patient-cluster bootstrap uncertainty intervals in the transfer analysis and for patient-grouped internal validation.
}\DIFaddend 
...
In the primary 24-hour shared-feature internal validation scenario on eICU, apparent performance was much more modest than in the secondary full-episode analysis. Logistic Regression produced the highest recall (0.586) but low precision (0.189), indicating many false-positive activations. Random Forest achieved high accuracy (0.912) and moderate AUC-ROC (0.765), but generated no positive predictions at the fixed 0.5 threshold; precision is therefore reported as NA rather than zero because TP+FP=0. Gradient Boosting showed similar discrimination (AUC-ROC \DIFaddbegin \DIFadd{0.732}\DIFaddend ) but very low recall (0.069).

In integrated five-fold cross-validation, AUC-ROC values remained in the range \DIFaddbegin \DIFadd{0.709}\DIFaddend --0.717, but threshold-dependent minority-class metrics were weak for tree-based models. Random Forest had recall $0.007 \pm 0.013$ and F1-score $0.013 \pm 0.026$, while Gradient Boosting had recall \DIFaddbegin \DIFadd{$0.080 \pm 0.045$ }\DIFaddend and F1-score \DIFaddbegin \DIFadd{$0.127 \pm 0.078$}\DIFaddend . Logistic Regression again favored sensitivity over precision, with recall $0.600 \pm 0.070$ and precision $0.196 \pm 0.023$. These values indicate that the clinically stricter landmark/shared-feature design substantially reduces the apparent performance observed in the secondary full-episode/full-feature results.
...
\begin{table}[h]
	...
	\begin{tabular}{p{140pt}p{50pt}p{50pt}p{50pt}p{45pt}p{45pt}p{60pt}}
		...
		Train eICU $\rightarrow$ Test MIMIC & GB & 0.709 & 0.667 & 0.061 & 0.111 & \DIFdelFL{0.609 }\DIFaddbeginFL \DIFaddFL{0.613 }\DIFaddendFL \\
		...
		Train MIMIC $\rightarrow$ Test eICU & GB & \DIFdelFL{0.755 }\DIFaddbeginFL \DIFaddFL{0.743 }\DIFaddendFL & \DIFdelFL{0.112 }\DIFaddbeginFL \DIFaddFL{0.109 }\DIFaddendFL & \DIFdelFL{0.256 }\DIFaddbeginFL \DIFaddFL{0.265 }\DIFaddendFL & \DIFdelFL{0.156 }\DIFaddbeginFL \DIFaddFL{0.154 }\DIFaddendFL & \DIFdelFL{0.555 }\DIFaddbeginFL \DIFaddFL{0.557 }\DIFaddendFL \\ \hline

	\end{tabular}
	\footnotesize
\end{table}

In the reverse scenario, from Train MIMIC $\rightarrow$ Test eICU, no model achieved clinically strong positive-class detection. Logistic Regression detected 44 of 117 deaths but produced 422 false positives. Random Forest preserved high specificity (0.971) but detected only 4 of 117 deaths. Gradient Boosting detected \DIFaddbegin \DIFadd{31 }\DIFaddend of 117 deaths but still had low precision (\DIFaddbegin \DIFadd{0.109}\DIFaddend ) and F1-score (\DIFaddbegin \DIFadd{0.154}\DIFaddend ). The comparison between directions supports a cautious interpretation: the central issue is weak transportable discrimination combined with instability of the probability scale and fixed-threshold decision rule.

Uncertainty in the primary transfer results was quantified using \DIFaddbegin \DIFadd{patient-cluster }\DIFaddend non-parametric bootstrap confidence intervals based on 2,000 resamples within each test set. \DIFaddbegin \DIFadd{Each bootstrap replicate sampled patient identifiers with replacement and included all ICU stays belonging to each selected patient, thereby accounting for repeated ICU stays where present. The MIMIC transfer test set contained }\DIFaddend 79 \DIFaddbegin \DIFadd{patient clusters }\DIFaddend and the eICU \DIFaddbegin \DIFadd{transfer test set contained }\DIFaddend 1,124 \DIFaddbegin \DIFadd{patient clusters}\DIFaddend . Precision is reported as NA when TP+FP=0. In the Train eICU $\rightarrow$ Test MIMIC scenario, Random Forest had sensitivity 0.000 (95\% \DIFaddbegin \DIFadd{cluster-bootstrap }\DIFaddend CI 0.000--0\DIFaddbegin \DIFadd{.000}\DIFaddend ), specificity 1.000 (95\% \DIFaddbegin \DIFadd{cluster-bootstrap CI 1.000}\DIFaddend --1.000), F1-score 0.000 (\DIFaddbegin \DIFadd{95\% cluster-bootstrap }\DIFaddend CI 0.000--0.000), and AUC-ROC 0.642 (\DIFaddbegin \DIFadd{95\% cluster-bootstrap CI 0.503}\DIFaddend --0\DIFaddbegin \DIFadd{.765}\DIFaddend ). Gradient Boosting had sensitivity 0.061 (95\% \DIFaddbegin \DIFadd{cluster-bootstrap CI 0.000}\DIFaddend --0\DIFaddbegin \DIFadd{.143}\DIFaddend ), specificity 0.987 (95\% \DIFaddbegin \DIFadd{cluster-bootstrap CI 0.956}\DIFaddend --1.000), F1-score 0.111 (\DIFaddbegin \DIFadd{95\% cluster-bootstrap }\DIFaddend CI 0.000--0\DIFaddbegin \DIFadd{.242}\DIFaddend ), and AUC-ROC \DIFaddbegin \DIFadd{0.613 (95\% cluster-bootstrap CI 0.461}\DIFaddend --0\DIFaddbegin \DIFadd{.755}\DIFaddend ). Logistic Regression had sensitivity 0.545 (95\% \DIFaddbegin \DIFadd{cluster-bootstrap CI 0.345}\DIFaddend --0\DIFaddbegin \DIFadd{.735}\DIFaddend ), specificity 0.571 (95\% \DIFaddbegin \DIFadd{cluster-bootstrap CI 0.424}\DIFaddend --0\DIFaddbegin \DIFadd{.687}\DIFaddend ), F1-score 0.429 (\DIFaddbegin \DIFadd{95\% cluster-bootstrap CI 0.260}\DIFaddend --0\DIFaddbegin \DIFadd{.575}\DIFaddend ), and AUC-ROC 0.598 (\DIFaddbegin \DIFadd{95\% cluster-bootstrap CI 0.459}\DIFaddend --0\DIFaddbegin \DIFadd{.720}\DIFaddend ). These uncertainty results are summarized in Table 6.

\begin{table}[h]
	\caption{Uncertainty summary for primary 24-hour shared-feature cross-domain transfer results. E2M denotes Train eICU $\rightarrow$ Test MIMIC; M2E denotes Train MIMIC $\rightarrow$ Test eICU. \DIFdelFL{Sensitivity and specificity }\DIFaddbeginFL \DIFaddFL{Bracketed }\DIFaddendFL intervals are \DIFdelFL{exact binomial }\DIFaddbeginFL \DIFaddFL{patient-cluster bootstrap }\DIFaddendFL 95\% confidence intervals \DIFdelFL{; accuracy, F1-score, AUC-ROC, and Brier intervals are stratified episode-level bootstrap 95\% intervals }from 2,000 resamples \DIFaddbeginFL \DIFaddFL{using \texttt{patient\_group\_id} as the cluster unit}\DIFaddendFL . Precision is reported as NA when TP+FP=0.}
...
\begin{table}[h]
	...
	\begin{tabular}{p{145pt}p{50pt}p{35pt}p{35pt}p{35pt}p{35pt}p{55pt}p{55pt}}
		...
		Train MIMIC $\rightarrow$ Test eICU & GB & \DIFdelFL{970 }\DIFaddbeginFL \DIFaddFL{953 }\DIFaddendFL & \DIFdelFL{237 }\DIFaddbeginFL \DIFaddFL{254 }\DIFaddendFL & \DIFdelFL{87 }\DIFaddbeginFL \DIFaddFL{86 }\DIFaddendFL & \DIFdelFL{30 }\DIFaddbeginFL \DIFaddFL{31 }\DIFaddendFL & \DIFdelFL{0.256 }\DIFaddbeginFL \DIFaddFL{0.265 }\DIFaddendFL & \DIFdelFL{0.804 }\DIFaddbeginFL \DIFaddFL{0.790 }\DIFaddendFL \\ \hline
