...
The explicit source mappings used for the primary shared-feature extraction are summarized in the unnumbered mapping table below. MIMIC laboratory variables were selected by item identifier from \texttt{LABEVENTS}, and vital signs were selected by item identifier from \texttt{CHARTEVENTS}. Creatinine was restricted to the blood/serum/plasma laboratory item represented by MIMIC item ID 50912 and the exact eICU laboratory name \texttt{creatinine}; urinary creatinine and protein/creatinine-ratio measurements were not mapped into the primary creatinine feature family. eICU laboratory variables were selected using exact lower-case lab names. All laboratory summaries were kept in the units supplied by the mapped source fields when clinically aligned; temperature was normalized to Celsius for the missingness audit but excluded from modeling.
